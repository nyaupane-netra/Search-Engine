...
def title_to_info(metadata):
    result = {}
    for data in metadata:
        result[data[0]] = {'author': data[1], 'timestamp': data[2], 'length': data[3]}
    return result
...
def article_length(max_length, article_titles, title_to_info):
    ...
    for titles in article_titles:
        if title_to_info[titles]['length'] <= max_length:
...
def filter_to_author(author, article_titles, title_to_info):
    titles_by_author = key_by_author(article_titles, title_to_info)

    if author in titles_by_author:
        return titles_by_author[author]
    else:
        return []
...
def filter_out(keyword, article_titles, keyword_to_titles):
    remove_articles = search(keyword, keyword_to_titles)
    result = []

    for titles in article_titles:
        if titles not in remove_articles:
            result.append(titles)
    return result
...
def articles_from_year(year, article_titles, title_to_info):
    year_start = time.mktime(datetime.date(year, 1, 1).timetuple())
    year_end = time.mktime(datetime.date(year + 1, 1, 1).timetuple())
    result = []
